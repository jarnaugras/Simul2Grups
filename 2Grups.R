      ...
      tabItem(tabName = "results",
              fluidRow(
                box(title = "Resultados de la Simulación", width = 12,
                    ...
                    downloadButton("downloadResultsPlot", "Descargar Gráfico")
                )
              )
      ),
      ...
      tabItem(tabName = "distributions",
              fluidRow(
                box(title = "Distribución de Datos", width = 6,
                    ...
                    downloadButton("downloadDataDist", "Descargar Gráfico")
                ),
                box(title = "QQ-Plot", width = 6,
                    plotOutput("qqPlot"),
                    downloadButton("downloadQQPlot", "Descargar Gráfico")
                )
              ),
              fluidRow(
                box(title = "Distribución de p-valores", width = 6,
                    plotOutput("pvaluesPlot"),
                    downloadButton("downloadPvaluesPlot", "Descargar Gráfico")
                ),
                box(title = "Bootstrap Distributions", width = 6,
                    plotOutput("bootstrapPlot"),
                    downloadButton("downloadBootstrapPlot", "Descargar Gráfico")
                )
              )
      )
    )
...
server <- function(input, output) {
    ...
    withProgress(message = 'Realizando simulaciones...', value = 0, {
      for (sim in 1:input$n_simul) {
        ...
        if (input$design_type == "paired") {
          ...
          yuen_result <- yuen.t.test(data1, data2, tr = input$trimming)
          if (yuen_result$p.value < input$alpha) {
            rechazos_yuen <- rechazos_yuen + 1
          }
  ...
  output$downloadResults <- downloadHandler(
    filename = function() {
      paste("resultados_simulacion_", Sys.Date(), ".csv", sep = "")
    },
  ...
  output$downloadResultsPlot <- downloadHandler(
    filename = function() {
      paste("grafico_resultados_", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      png(file)
      res <- results()
  ...
  output$downloadDataDist <- downloadHandler(
    filename = function() {
      paste("distribucion_datos_", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
  ...
  output$downloadQQPlot <- downloadHandler(
    filename = function() {
      paste("qqplot_", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      png(file)
      data <- sample_data()
  ...
  output$downloadPvaluesPlot <- downloadHandler(
    filename = function() {
      paste("pvalues_plot_", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
  ...
  output$downloadBootstrapPlot <- downloadHandler(
    filename = function() {
      paste("bootstrap_plot_", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
